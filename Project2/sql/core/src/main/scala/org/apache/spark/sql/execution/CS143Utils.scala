...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    def UdfAch(exp: Seq[Expression]): ScalaUdf = {
      exp match {
       case head :: tail => head match {
          case u: ScalaUdf => u
          case otherwise => UdfAch(tail)
         }
       case Nil      => null
       }
    }
    UdfAch(expressions.reverse)
  }
  ...
  def apply(
    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          if(input.hasNext) {
	    val nextRow: Row = input.next()
	    val key: Row = cacheKeyProjection.apply(nextRow)
	    val pre: Row = preUdfProjection.apply(nextRow)
	    
	    val check: Row = {
	      if (cache.containsKey(key)) {
	        cache.get(key)
	      } else {
	        val add: Row = udfProject.apply(nextRow)
                cache.put(key, add)
		add
	      }
	    }

	    val post: Row = postUdfProjection.apply(nextRow)
	    Row.fromSeq(pre ++ check ++ post)
	    } else {
              null
	  }
        }
