...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENTED
    val diskPartitions = DiskHashedRelation.apply(input, keyGenerator).getIterator()
    ...
    new Iterator[Row] {
      ...
      private def fetchNextPartition(): Boolean  = {
        // IMPLEMENTED
        if (!diskPartitions.hasNext) false
        else {
          it = CS143Utils.generateCachingIterator(projectList, child.output)(diskPartitions.next().getData())
          hasNext()
        }
      }
    }
