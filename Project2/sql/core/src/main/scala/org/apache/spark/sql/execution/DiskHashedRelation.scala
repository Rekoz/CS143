...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    // IMPLEMENTED
    if (inputClosed) throw new SparkException("inputClosed")
    data.add(row)
    val estSize = measurePartitionSize()
    if (estSize > blockSize) {
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      private[this] def fetchNextChunk(): Boolean = {
        // IMPLEMENTED
        if (chunkSizeIterator.isEmpty) false
        else {
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    // IMPLEMENTED
    var i = -1
    val partitions = Array.fill(size) {
      i += 1
      new DiskPartition("block"+i.toString(), blockSize)
    }
    while (input.hasNext) {
      var row = input.next()
      var hashRes = keyGenerator(row).hashCode() % size
      partitions(hashRes).insert(row)
    }
